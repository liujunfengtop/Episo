/* methylation_ratio.c
   ...
   methylation_ratio <isforms.fpkm_tracking> <isforms_methylation.fpkm_tracking> <total_number> <methylation_number>
...
int ratio (char *isoform, char *isoform_methylation, char *isoform_filter, char *isoform_methylation_filter, char *total_number, char *methylation_number);

int main (int argc, char* argv[])
{
   int l;
   
   char isoform[1000], isoform_methylation[1000], isoform_filter[1000], isoform_methylation_filter[1000], total_number[1000], methylation_number[1000];

   if(argc>6) {
   	strcpy(isoform, argv[1]);
   	strcpy(isoform_methylation, argv[2]);
   	strcpy(isoform_filter, argv[3]);
   	strcpy(isoform_methylation_filter, argv[4]);
   	strcpy(total_number, argv[5]);
   	strcpy(methylation_number, argv[6]);
   } else {
   	return -1;
   }

   l=ratio(isoform,isoform_methylation,isoform_filter,isoform_methylation_filter,total_number,methylation_number);
...
int ratio (char *isoform, char *isoform_methylation, char *isoform_filter, char *isoform_methylation_filter, char *total_number, char *methylation_number)
{
  ...
  */
	...
	/* n for total fpkm from the isoform file;
	...
	*/
	...
	/* m for total fpkm from the isoform_methylation file;
	...
	*/
	...
	FILE *fisoform, *fisoform_methylation, *fisoform_filter, *fisoform_methylation_filter, *fout;
	  ...
	  fgets(ls1,5000,fisoform_methylation_filter);
	 	while(read1<12) {
	 		...
	 		if(ls1[j1]==' '||ls1[j1]=='\t'||ls1[j1]=='\n') {
	 			...
	 			if(read1==11) {
	 				m3=atof(temp1);
	 				if((n1/n)==0.0||pow(n1/n,2)==0.0||pow(n1/n,4)==0.0) {
	 					fprintf(fout,"%s\t%s\t%s\t%f\t%f\t%f\n",temp2,temp3,temp4,0.0,0.0,0.0);
	 				} else {
	 					//ratio_estimated=((m1/m)*atof(methylation_number))/((n1/n)*atof(total_number)); // 2016-03-09 commented by ljf because the form was error
	 					ratio_estimated=(atof(methylation_number)/atof(total_number))*((m1/m)/(n1/n)+((m1/m)*pow(((n3-n1)/n)/1.96,2))/pow(n1/n,3)); //2016-03-09 by ljf corrected
	 					if(ratio_estimated>1.0) {
	 						ratio_estimated=1.0;
	 					}
	 					ratio_estimated_lo=ratio_estimated-1.96*(atof(methylation_number)/atof(total_number))*sqrt(pow(((m3-m1)/m)/1.96,2)/pow(n1/n,2)+pow(m1/m,2)*pow(((n3-n1)/n)/1.96,2)/pow(n1/n,4));
	 					if(ratio_estimated_lo<0.0) {
	 						ratio_estimated_lo=0.0;
	 					}
	 					ratio_estimated_hi=ratio_estimated+1.96*(atof(methylation_number)/atof(total_number))*sqrt(pow(((m3-m1)/m)/1.96,2)/pow(n1/n,2)+pow(m1/m,2)*pow(((n3-n1)/n)/1.96,2)/pow(n1/n,4));
