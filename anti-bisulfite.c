...
#include<errno.h>
#include<stdlib.h>
/*include ctype.h because of implicit declaration of function on linux by ljf 2016-07-26*/
#include<ctype.h>
...
FILE *fout, *foutread, *foutread1, *foutread2, *fintxt, *finmultxt, *fsummary;

int main (int argc, char* argv[])
{
   char VerStr[32] = "Version 1.0, Septemper 2015";
   ...
   /*i for the skipped rows from the txt file;
     ...
     l for 'for loop';
     count for the number of the methylation site in one segment;
   */
   int i,j,read,k,l,count;
   ...
   */
   ...
   strcpy(com.ctlf, "anti-bisulfite.ctl");
   ...
   if((fsummary=fopen("methylation_summary", "w"))==NULL) {
   		printf("Open %s file Error: %s\n","methylation_summary",strerror(errno));
   		fprintf(fout,"Open %s file Error: %s\n","methylation_summary",strerror(errno));
   		return -1;
   }
   ...
   if((finmultxt=fopen(com.inmultxtf,"r"))==NULL) {
  	  printf("Open %s file Error: %s\n",com.inmultxtf,strerror(errno));
   		fprintf(fout,"Open %s file Error: %s\n",com.inmultxtf,strerror(errno));
   		return -1;
   }
   ...
   printf("anti-bisulfite (%s)\n", VerStr);
   fprintf(fout, "anti-bisulfite (%s)\n", VerStr);
                        ...
                        if(read==10) {
                          for(l=0;l<k;l++) {
                        	...
                        	if(tag=='1') {
                        		...
                        		for(l=0;l<strlen(ls1)-1;l++) {
                        			fprintf(fsummary,"%c",ls1[l]);
                        		}
                        		fprintf(fsummary,"\t%d\n",count);
                        	}
                        }                        
